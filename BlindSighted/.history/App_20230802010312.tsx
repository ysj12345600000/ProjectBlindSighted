...
import { useBleManager, BleManagerContextProvider, DeciceInfo, RssiInfo } from './components/BleOperation2';
 ...
 // 用于定时器
  ...
  useEffect(() => {
    const fetchAngleAndPosition = ()=>{
      ...
      if(connectedDevicesNumber < 3) {
        console.log('请连接至少三个设备');
        return;
      }

      if(!connectedDevicesInfo[0].distanceCalculateParams) {
        //处理不完整的设备信息。
        console.log('设备信息数量不足，无法计算距离');
        const macId = connectedDevicesInfo.find(deviceInfo => deviceInfo.index === 1)?.device.id;
        if(!macId) {setIsSearchBle(true);return;} // 如果没有找到macId,则重新扫描
        setCurrentCommunicatrMacId(macId);
        return;
      }

      //确认Rssi信息数量是否足够
      if(rssiLists.some(rssiInfo => rssiInfo.rssiList.length < 50)) {
        console.log('Rssi信息数量不足，无法计算距离');
    ...
    fetchAngleAndPosition();
    },[calculateForDistance, connectedDevicesInfo, rssiLists, connectedDevicesNumber]);
  ...
  const calculateDistancePositionAngle = async ( deviceInfoList : DeciceInfo[], rssiLists : RssiInfo[]) => {
  ...
  const renderDeviceInfoDisp= ( {item}: {item: DeviceInfoDisp}) => {
    ...
    try{
    if(item.device) {
      if(item.device.name) displayName = item.device.name;
      if(item.device.id) macId = item.device.id;
    }}catch(e) {
      console.log(e);
    }
    ...
    return (
      ...
      <View style={{flexDirection: 'column', alignItems: 'center'}}>
        ...
        <Text style={styles.itemText}>设备ID: {macId}</Text>
  ...
  return (
    <View style={styles.container}>
      <Button title="Scan Devices" color={isSearchBle? 'red': 'blue'}onPress={()=>{setIsSearchBle(!isSearchBle)}} />
      <Button title="获取RSSI值" color='green' onPress={()=>{setIsAllowRssiScan(!isAllowRssiScan)}} />
      ...
      <Button title="信息" color='black' onPress={()=>{notiy()}} />
