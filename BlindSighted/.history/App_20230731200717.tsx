 ...
 */
  ...
  const calculateDistancePositionAngle = async ( deviceInfoList : ConnectedDeciceInfo[], rssiLists : RssiInfo[]) => {
    ...
    const usersPosition = await getUsersPosition(updateDeviceInfo, userHight);

    if(!usersPosition) return;

    console.log('用户坐标', usersPosition);

    const angle = await angleDifference(updateDeviceInfo, {x: usersPosition![0], y: usersPosition![1]});

    if(!angle) return;

    console.log('用户离目标角度', angle);

    setUserPosition(usersPosition);
