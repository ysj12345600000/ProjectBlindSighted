...
export const BleManagerContextProvider = ({ children }: BleManagerContextProviderProps) => {
    ...
    const connectBle = async (macId:string) => {
        ...
        try{
            ...
            setRssiLists(prevRssiLists => {
                const updatedRssiLists = prevRssiLists.map((rssiList, index) => {
                    const parts = device.name!.split('_');
                    const numberPart = parseInt(parts[parts.length - 1]);
                    if(numberPart === index + 1){
                        return {
                            ...rssiList,
                            index: numberPart,
                            macId: macId,
    ...
    const addRssiToList = (id: string, nameIndex : number, rssi: number) => {
        
        setRssiLists(prevRssiLists => {
            const updatedRssiLists = prevRssiLists.map((rssiList) => {
                if(rssiList.index === nameIndex){
                    if(rssiList.rssiList.length >= MAX_RSSI_LENGTH){
                        rssiList.rssiList.shift();
                    }
                    return {
                        ...rssiList,
                        rssiList: [...rssiList.rssiList, rssi],
                    };
                }
            });
