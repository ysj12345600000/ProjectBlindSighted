        ...
        const executeBleOperations = async () => {
            if (currentCommunicatrMacId) {
                ...
                if (deviceInfo) {
                    ...
                    const removeMonitorSubscription = await startNoticeBle(deviceInfo);
                    await bleWirte(deviceInfo, 'GETDEVICEINFO');
    
                    return removeMonitorSubscription; // 返回移除订阅的函数
                }
            }
        };
    
        const removeMonitorSubscription = executeBleOperations();
