...
export const BleManagerContextProvider = ({ children }: BleManagerContextProviderProps) => {
    ...
    const startSearchBle = () => {
        // console.log('开始搜索蓝牙');
        if (isSearchBle) {
            console.warn('A scanning operation is already in progress.');
            return;
         }
        setIsSearchBle(true);
    };
    ...
    const startNoticeBle = (deviceInfo: ConnectedDeciceInfo) => {
    ...
    useEffect(() => {
        let removeMonitorSubscription: (() => void) | undefined;
        const executeBleOperations = async () => {
            if (currentCommunicatrMacId) {
                ...
                if (deviceInfo) {
                    ...
                    if (!deviceInfo.notifyCharacteristicUUID) {
                        await getServiceID(currentCommunicatrMacId);
                    }
    
                    removeMonitorSubscription = await startNoticeBle(deviceInfo);
        ...
        return () => {
            if (removeMonitorSubscription) removeMonitorSubscription(); // 卸载阶段移除订阅
        }
