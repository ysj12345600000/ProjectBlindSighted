...
export const BleManagerContextProvider = ({ children }: BleManagerContextProviderProps) => {
    ...
    const startNoticeBle = async (deviceInfo: ConnectedDeciceInfo) => {
        ...
        const subscription = manager.monitorCharacteristicForDevice(macId, deviceInfo.serviceId, deviceInfo.notifyCharacteristicUUID, (error, characteristic) => {
            if (error) {
                ...
                console.log('监听失败', macId, deviceInfo.serviceId, deviceInfo.notifyCharacteristicUUID);
                return;
            }
            if(characteristic){
    ...
    }, [currentCommunicatrMacId, connectedDevicesInfo.find(deviceInfo => deviceInfo.index === 1)?.notifyCharacteristicUUID,connectedDevicesInfo.find(deviceInfo => deviceInfo.index === 1)?.writeCharacteristicUUID]);
