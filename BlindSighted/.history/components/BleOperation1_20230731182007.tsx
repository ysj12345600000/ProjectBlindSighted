...
export const BleManagerContextProvider = ({ children }: BleManagerContextProviderProps) => {
    ...
    const [currentCommunicatrMacId, setCurrentCommunicatrMacId] = useState<string|null>('');
    ...
    //设备通讯
    const getDeviceInfo = async (macId: string): Promise<ConnectedDeciceInfo | undefined> => {
        let retries = 0;
    
        const tryGetDeviceInfo = (): Promise<ConnectedDeciceInfo | undefined> => {
            return new Promise((resolve) => {
                setTimeout(() => {
                    const info = connectedDevicesInfo.find(device => device.device.id === macId);
                    resolve(info);
                }, RETRY_DELAY);
            });
        };
    
        let deviceInfo = connectedDevicesInfo.find(device => device.device.id === macId);
    
        while (!deviceInfo && retries < MAX_RETRIES) {
            deviceInfo = await tryGetDeviceInfo();
            retries++;
        }
