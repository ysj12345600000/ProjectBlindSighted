...
import { DeciceInfo, RssiInfo } from '../components/BleOperation2';
...
export const triangulationCalculater = () => {
    ...
    const calculateDistances = async (connectDevicesInfoList: DeciceInfo[], rssiLists: RssiInfo[]) => {
    ...
    // 最小二乘法获取用户位置
    const getUsersPosition = async (connectDevicesInfoList: DeciceInfo[], userHeight: number) => {
    ...
    // 非线性优化获取用户位置
    function triangulationOptimization(connectDevicesInfoList: DeciceInfo[]) {
    ...
    const angleDifference = async (connectDevicesInfoList : DeciceInfo[], userPosition: {x:number, y:number}) => {
        ...
        const targetDeviceParams1 = connectDevicesInfoList.filter( info=>{
            if(info.index === 1){
                return info;
            }
        });

        const targetDeviceParams2 = connectDevicesInfoList.filter( info=>{
            if(info.index === 2){
                return info;
            }
        });
