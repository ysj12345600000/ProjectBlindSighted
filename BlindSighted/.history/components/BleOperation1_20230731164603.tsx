    ...
    useEffect(() => {
        if(connectedDevicesInfo.length === 3&&distanceCalculateParamsList.length === 3){
            setConnectedDevicesInfo(prevDevices => prevDevices.map(deviceInfo=>{
                const diatanceCalculateParams = distanceCalculateParamsList.find(params => params.macId === deviceInfo.device.id)!;
                if(!diatanceCalculateParams){return deviceInfo}
                return{
                    ...deviceInfo,
                    distanceCalculateParams : diatanceCalculateParams,
                };
            }
            ));
        }
