...
export const BleManagerContextProvider = ({ children }: BleManagerContextProviderProps) => {
    ...
    const addRssiToList = (macId: string, nameIndex : number, rssi: number) => {
        setRssiLists(prevRssiLists => {
            const updatedRssiLists = prevRssiLists.map((rssiList, index, macId) => {
                if (index === nameIndex&&) {
                    return [...rssiList, {index : nameIndex, macId : macId, rssi : rssi}];
                }
                return rssiList;
            });
