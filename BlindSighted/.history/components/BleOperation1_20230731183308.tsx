...
export const BleManagerContextProvider = ({ children }: BleManagerContextProviderProps) => {
    ...
    useEffect(() => {
        if(currentCommunicatrMacId){
            ...
            if(deviceInfo){
                (async () => {
                if(!deviceInfo.notifyCharacteristicUUID) getServiceID(currentCommunicatrMacId);
                startNoticeBle(deviceInfo);
                bleWirte(deviceInfo, 'GETDEVICEINFO');
            })();
            }   
        }
    }, [currentCommunicatrMacId, connectedDevicesInfo]);
