...
export const BleManagerContextProvider = ({ children }: BleManagerContextProviderProps) => {
    ...
    const startSearchBle = () => {
        if(connectedDevicesNumber >= 3) return;
        console.log('开始搜索蓝牙');
        setIsSearchBle(true);
    };
    ...
    const connectBle = async (macId:string) => {
        ...
        try{
            ...
            setConnectedDevicesInfo(prevDevices => {
                if(prevDevices.some((device_ => device_.index === numberPart))){
    ...
    const startNoticeBle = async(deviceInfo: DeciceInfo) => {
        ...
        const decodeFormatMessage2DsitanceCalculateParams = (message: string) => {
            ...
            setConnectedDevicesInfo(prevDevices =>{
                if(prevDevices.some((device => device.index === numberPart))){
                    return prevDevices.map(deviceInfo=>{
                        if(deviceInfo.index === numberPart){
                            return {
                                ...deviceInfo,
                    ...
                    const newDeviceInfo = {
                        device : null,
                        index : numberPart,
                        notifyCharacteristicUUID: deviceInfo.notifyCharacteristicUUID,
                        serviceId: deviceInfo.serviceId,
                        writeCharacteristicUUID: deviceInfo.writeCharacteristicUUID,
                        responeseContext : '',
                        distanceCalculateParams : distanceCalculateParams,
                    };
                    return [...prevDevices, newDeviceInfo];
            ...
            // setConnectedDevicesInfo(prevDevices => prevDevices.map(deviceInfo=>{
            //     if(deviceInfo.device.name === deviceName){
            //         return {
            //             ...deviceInfo,
            //             distanceCalculateParams : distanceCalculateParams,
            //         };
