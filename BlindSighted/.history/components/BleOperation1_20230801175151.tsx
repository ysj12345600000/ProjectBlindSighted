...
export const BleManagerContextProvider = ({ children }: BleManagerContextProviderProps) => {
    ...
    useEffect(() => {
        console.log('isSearchBle', isSearchBle);
    ...
    useEffect(() => {
        if(connectedDevicesInfo[0].distanceCalculateParams) return;
        console.log('设备信息数量足够，开始分配数据0', connectedDevicesInfo, distanceCalculateParamsList);
        if (connectedDevicesInfo.length >= 3 && distanceCalculateParamsList.length >= 3) {
            console.log('设备信息数量足够，开始分配数据', connectedDevicesInfo, distanceCalculateParamsList);
            ...
            console.log('设备信息数量足够，开始分配数据1', updatedDevices);
            setConnectedDevicesInfo(updatedDevices);
        }
    }, [connectedDevicesInfo, distanceCalculateParamsList]);

    const notiy=() => {
        console.log('distanceCalculateParamsList', distanceCalculateParamsList);
    ...
    const startNoticeBle = async (deviceInfo: ConnectedDeciceInfo) => {
        const decodeFormatMessage2DsitanceCalculateParams = (macId : string, rescontext: string) => {
            ...
            setDistanceCalculateParamsList(prevParams => {
                ...
                if(prevParams.some(params => params.deviceName === dsitanceCalculateParams.deviceName)){
                    return prevParams;
                }
